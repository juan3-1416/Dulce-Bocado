...
export default function IngresoDetail() {
  ...
  const cargarDetalle = async () => {
    try {
      setCargando(true)
      const data = await obtenerIngreso(id)
      setIngreso(data)
    } catch (err) {
      setError(err.message || 'Error al cargar el detalle del ingreso.')
    } finally {
      setCargando(false)
    }
  }
      ...
      return (
        <div>
          ...
          <div className="text-gray-500 text-xs">
            {detalle.producto_presentacion.nombre}
          </div>
        </div>
      )
    }
              ...
              <table className="min-w-full divide-y divide-gray-300">
                ...
                <tbody className="divide-y divide-gray-200 bg-white">
                  {ingreso.detalles.map((detalle) => (
                    <tr key={detalle.id_detalle_ingreso}>
                      ...
                      <td className="whitespace-nowrap px-3 py-4 text-sm text-right font-semibold text-gray-900 sm:pr-6">
                        {Number(detalle.cantidad).toLocaleString('es-ES', { maximumFractionDigits: 2 })}
