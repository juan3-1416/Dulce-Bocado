...
/**
 * Panel completo que se muestra mientras existe una transacción QR activa.
 *
 * Maneja los tres estados del QR: PENDIENTE, APROBADO y VENCIDO.
 *
 * @param {object}      props
 * @param {object}      props.transaccionQr
 * @param {string}      props.error
 * @param {string}      props.urlQr
 * @param {boolean}     props.qrPendiente
 * @param {boolean}     props.qrAprobado
 * @param {boolean}     props.qrVencido
 * @param {number}      props.segundosRestantes
 * @param {boolean}     props.consultandoQr
 * @param {Function}    props.formatearTiempo
 * @param {Function}    props.generarOtroQr
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="text-center">
        ...
        <p className="mt-2 text-3xl font-bold text-gray-900">
          Bs{' '}
          {Number(transaccionQr.monto).toFixed(2)}
        </p>
      </div>
      ...
      {window.location.hostname === 'localhost' && (
        <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">
          Para escanear desde otro dispositivo
          posteriormente configuraremos la dirección
          de red local. El flujo puede probarse desde
